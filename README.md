# AVD 카카오톡 봇 (메신저봇R v40)

메신저봇R v40 (Android)에서 실행되는 카카오톡 봇 스크립트입니다. 백엔드 서버와 연동하여 일정 알림 및 명령어 처리를 수행합니다.

## ⚠️ 중요: 메신저봇R 버전

**권장 버전: v40 (0.7.40.x)**
- Play 스토어 버전(v29)은 레거시 버전으로 비권장
- kbotdocs.dev → 메신저봇R → "안정 버전" 링크에서 최신 버전 다운로드 권장
- API2 하이브리드 모드 지원 (API1 이벤트 + API2 메시지 전송, v39 호환)
...
### 3. 봇 스크립트 업로드
1. 메신저봇R 앱 실행
2. 새 봇 생성 (API2 또는 레거시 API 선택 가능)
3. **`bot.js` 파일 복사/붙여넣기**
4. 설정 값 입력 (아래 참조)
5. 저장 및 컴파일
6. 봇 활성화 토글 ON
...
> 💡 **동적 명령어**: `/`로 시작하는 모든 메시지가 백엔드로 전달됩니다. 새 명령어 추가 시 **bot.js 수정 불필요**, 백엔드만 수정하면 됩니다.
...
- **언어**: JavaScript (ES5)
- **실행 환경**: 메신저봇R v40 (Rhino 엔진)
- **API**: API2 하이브리드 (BotManager + response 폴백)
...
### v1.2 (2026-02-14)
- ✅ 동적 명령어: COMMANDS whitelist 제거, `/`로 시작하는 모든 메시지 백엔드 전달
- ✅ 새 명령어 추가 시 bot.js 수정 불필요 (백엔드만 수정)
- ✅ callBackendAPI에 X-BOT-TOKEN 헤더 추가
